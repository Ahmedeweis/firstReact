...
export function OrderDetailsHistory({ history }: Props) {
  ...
  const renderSummary = (
    ...
    >
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>{t('orderDetails.orderTime')}</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>{t('orderDetails.paymentTime')}</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>{t('orderDetails.deliveryTime')}</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>{t('orderDetails.completionTime')}</Box>
        {fDateTime(history?.orderTime)}
      </Stack>
    </Paper>
  );
  ...
  return (
    <Card>
      <CardHeader title={t('orderDetails.history')} />
      <Stack
        spacing={3}
